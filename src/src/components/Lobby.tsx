...
import { Users, Plus, Link, ArrowLeft, Copy, Check, Shield, Swords, Play, Rocket, User, Zap, Loader2 } from 'lucide-react';
...
const Lobby: React.FC<LobbyProps> = ({ onGameStart, onBack, returnRoomId, returnPlayerName }) => {
    ...
    const ensureConnected = (callback: () => void) => {
        if (socket.connected) {
            ...
            setTimeout(() => {
                if (!socket.connected) {
                    setIsConnecting(false);
                    setError('Connection timeout. Please check your internet or if the server is running.');
                }
            }, 5000);
        }
    };
    ...
    const handleCreateRoom = () => {
        ...
        ensureConnected(() => {
            ...
            // Safety timeout so we don't spin forever
            setTimeout(() => {
                setIsCreatingRoom((prev) => {
                    if (prev) setError('Room creation timed out. Please try again.');
                    return false;
                });
            }, 10000);
        });
            ...
            case 'create':
                return (
                    <div className="max-w-lg w-full bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-2xl">
                        ...
                        {error && (
                            <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold py-3 px-4 rounded-xl mb-4 flex items-center gap-2">
                        ...
                        <div className="flex gap-4">
                            <button
                                ...
                                className="flex-1 py-4 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-30 text-zinc-300 rounded-2xl font-bold transition-all cursor-pointer"
                            >
                                Back
                            </button>
                                <button
                                    onClick={handleCreateRoom}
                                    disabled={!!lobbyCountdown || isConnecting || isCreatingRoom}
                                    className="flex-[2] py-4 bg-green-600 hover:bg-green-500 disabled:opacity-30 disabled:cursor-not-allowed text-white rounded-2xl font-bold shadow-lg shadow-green-600/20 animate-in fade-in slide-in-from-bottom-2 transition-all cursor-pointer flex items-center justify-center gap-2"
                                >
                                    {isConnecting || isCreatingRoom ? (
                                        <>
                                            <Loader2 size={20} className="animate-spin" />
                                            <span>{isConnecting ? 'Linking...' : 'Creating...'}</span>
                                        </>
                                    ) : (
                                        'Create'
                                    )}
                            ...
                            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-xl"
                                ...
                                <div className="flex flex-col items-center gap-6">
                                    ...
                                    <div className="relative w-32 h-32 flex items-center justify-center">
                                        ...
                                        {/* Spinning arc */}
                                        <div className="absolute inset-0 rounded-full border-2 border-transparent border-t-green-400 border-r-green-400/50"
                                             style={{ animation: 'spin 1.2s ease-in-out infinite' }} />
                                        {/* Inner glow ring */}
                                        <div className="absolute inset-3 rounded-full border border-green-500/10"
                                             style={{ animation: 'spin 3s linear infinite reverse' }} />
                                        <div className="absolute inset-3 rounded-full border border-transparent border-b-green-300/40"
                                             style={{ animation: 'spin 1.8s ease-in-out infinite' }} />
                                        {/* Center icon */}
                                        <div className="relative w-16 h-16 bg-green-500/10 rounded-2xl flex items-center justify-center"
                                    ...
                                    <div className="text-center">
                                        <p className="text-green-400 font-black uppercase tracking-[0.3em] text-sm"
                                           style={{ animation: 'pulse 1.5s ease-in-out infinite' }}>
                                            Deploying Room
                                        </p>
                                        <p className="text-zinc-500 text-xs mt-2 font-medium">
                                            Establishing connection to the battle server...
                                        </p>
                                    </div>
                                ...
                                {/* Scanline effect */}
                                <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.15)_50%),linear-gradient(90deg,rgba(0,255,100,0.03),rgba(0,255,0,0.01),rgba(0,255,100,0.03))] bg-[length:100%_2px,3px_100%] z-[201]" />
                            </div>
                        )}
                    </div>
                );
                        ...
                        <div className="flex gap-4">
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className="flex-[2] py-4 bg-blue-600 hover:bg-blue-500 disabled:opacity-30 disabled:cursor-not-allowed text-white rounded-2xl font-bold shadow-lg shadow-blue-600/20 transition-all cursor-pointer flex items-center justify-center gap-2"
                                >
                                    {isConnecting ? (
                                        <>
