...
import { PvpEngine } from '../game/PvpEngine';
...
export function PvpGameCanvas({ biome, onGameOver, multiplayerInfo }: GameCanvasProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<PvpEngine | null>(null);
  ...
  useEffect(() => {
    ...
    // Find my index
    const myIndex = multiplayerInfo?.players.findIndex((p: any) => p.name === multiplayerInfo.playerName) ?? 0;
    const finalIndex = myIndex >= 0 ? myIndex : 0;
    
    engineRef.current = new PvpEngine(biome, targetScore, mapSeed, finalIndex);
